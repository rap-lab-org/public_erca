...
/**
 * @brief Adds degree cost as the 'cost_index'th element of the cost vector.
...
/**
 * @brief Adds a unit cost as the 'cost_index'th element of the cost vector.
 */
void Roadmap::AddUnitCost(int cost_index) {
  for (const auto& kv1: _adjlist) {
    long u = kv1.first;
    for (const auto& kv2: _adjlist[u]) {
      long v = kv2.first;
      _adjlist[u][v][cost_index] = 1;
      _adjlist_rev[v][u][cost_index] = 1;
    }
  }
};


basic::CostVector GetPathCostInGraph(Graph* g, std::vector<long>& path)
{
  basic::CostVector cvec;
  cvec.resize(g->GetCostDim(), 0);
  if (path.size() != 0){
    for (size_t i = 0; i < path.size()-1; i++){
      cvec += g->GetCost(path[i],path[i+1]);
    }
  }
  return cvec;
};
