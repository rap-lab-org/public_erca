...
public:
  ...
  virtual void AddDegreeCost(int cost_index);
    ...
    // for each edge, the value of its cost vector at index 'cost_index' is set to be a unit cost.
  virtual void AddUnitCost(int cost_index);
...
/**
 * @brief Return the path cost in graph g.
 */
